...
<head>
  ...
  <title>TodoMVC using Servlets / JSP</title>
...
<section id="todoapp">
  ...
  <footer id="footer" class="empty-${todos.isEmpty()}">
    <span id="todo-count"><strong><c:out value="${todos.size() - completed.size()}" /></strong> item<c:if test="${todos.size() - completed.size() != 1}">s</c:if> left</span>
